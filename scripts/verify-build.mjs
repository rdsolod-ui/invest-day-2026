...
for(const text of ['249','455,6','76,7','2027','2028','14.09','Python','RU000A109PK2','Тёмная','Системная'])assert.ok(html.includes(text),`Missing ${text}`);
...
for(const id of ['speaker-viktor','industry-location','speaker-ivan','scale','speaker-natalia','investment','speaker-alexey','discussion'])assert.ok(sceneIds.includes(id));
for(const bad of ['152,3','13,3%','40%','65%','30% повторных','Слабый сезон','миллионов гостей'])assert.ok(!html.includes(bad),`Removed claim remains: ${bad}`);
