...
scenes.forEach((scene,i)=>{const label=scene.querySelector('.stage-index');if(label)label.textContent=`${String(i+1).padStart(2,'0')} / ${scenes.length}`});
...
function animateScene(scene:HTMLElement){
 ...
 const flows=Array.from(scene.querySelectorAll<HTMLElement>('[data-flow]'));flows.forEach((el,i)=>{const eventId=Number(el.dataset.eventId||el.dataset.journalId);const revealDelay=eventId?1800+(eventId-1)*600:i*650;list.push(el.animate([{opacity:0},{opacity:1}],{duration:700,delay:revealDelay,fill:'backwards',easing:'ease-out'}));if(!scene.classList.contains('finance-scene'))list.push(el.animate([{filter:'brightness(1)'},{filter:'brightness(1.12)'},{filter:'brightness(1)'}],{duration:7000,delay:1400+i*850,iterations:Infinity,easing:'ease-in-out'}));});
...
const historyScene=document.querySelector<HTMLElement>('#baseline')!;
const calendarScene=document.querySelector<HTMLElement>('#calendar')!;
const historyLoop:Loop={name:'history',scene:historyScene,size:historyScene.querySelectorAll('[data-model-year]').length,index:0,paused:false,timer:null,animations:[],duration:4200,step(i,animate){
 historyScene.querySelectorAll<HTMLElement>('[data-model-hint]').forEach((el,j)=>el.hidden=i!==j);
 historyScene.querySelectorAll<HTMLElement>('[data-model-year]').forEach((el,j)=>el.setAttribute('aria-pressed',String(i===j)));
 historyScene.querySelectorAll('[data-model-point]').forEach(el=>el.classList.toggle('selected',Number((el as HTMLElement).dataset.modelPoint)===i));
 if(animate){const hint=historyScene.querySelector<HTMLElement>(`[data-model-hint="${i}"]`)!;this.animations.push(hint.animate([{opacity:.25},{opacity:1}],{duration:500,easing:'ease-out'}));}
}};
const calendarLoop:Loop={name:'calendar',scene:calendarScene,size:calendarScene.querySelectorAll('[data-event]').length,index:0,paused:false,timer:null,animations:[],duration:4200,step(i,animate){
 calendarScene.querySelectorAll<HTMLElement>('[data-event]').forEach((el,j)=>{el.classList.toggle('selected',i===j);el.setAttribute('aria-pressed',String(i===j));});
 calendarScene.querySelectorAll<HTMLElement>('[data-event-hint]').forEach((el,j)=>el.hidden=i!==j);
 if(animate){const line=calendarScene.querySelector<HTMLElement>(`[data-event="${i}"] .event-progress`)!;this.animations.push(line.animate([{transform:'scaleX(0)'},{transform:'scaleX(1)'}],{duration:this.duration,easing:'linear'}));}
}};
...
document.querySelectorAll<HTMLElement>('[data-model-year]').forEach((el,i)=>{el.addEventListener('click',()=>selectMilestone(historyLoop,i));el.addEventListener('focus',()=>selectMilestone(historyLoop,i));el.addEventListener('pointerenter',e=>{if(e.pointerType==='mouse')selectMilestone(historyLoop,i);});});
...
let frame=false;function updateScroll(){frame=false;if(presenting)return;const max=root.scrollHeight-innerHeight;progress.style.transform=`scaleX(${max>0?Math.min(1,scrollY/max):0})`;let active='';for(const id of ['industry-location','product','investment','baseline'])if(document.getElementById(id)!.getBoundingClientRect().top<innerHeight*.45)active=id;document.querySelectorAll<HTMLAnchorElement>('.main-nav a').forEach(a=>{if(a.hash==='#'+active)a.setAttribute('aria-current','location');else a.removeAttribute('aria-current')});}addEventListener('scroll',()=>{if(!frame){frame=true;requestAnimationFrame(updateScroll)}},{passive:true});updateScroll();
if(location.hash==='#targets')history.replaceState(null,'','#baseline');
addEventListener('hashchange',()=>{if(location.hash==='#targets'){history.replaceState(null,'','#baseline');if(presenting)renderScene(scenes.indexOf(historyScene));else historyScene.parentElement?.scrollIntoView({behavior:'instant'});}});
